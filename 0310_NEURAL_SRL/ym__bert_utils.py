...
def data_to_tensors(dataset: List, tokenizer: BertTokenizer, pred_info: List, max_len: int, labels: List=None, label2index: Dict=None, pad_token_label_id: int=-100) -> Tuple:
    ...
    input_preds = pad_sequences(pred_info, maxlen=max_len, dtype="long", value=0, truncating="post", padding="post")
    if label_indices:
    ...
    return LongTensor(input_ids), LongTensor(attention_masks), label_ids,  LongTensor(seq_lengths), LongTensor(input_preds)
...
def evaluate_bert_model(eval_dataloader: DataLoader, eval_batch_size: int, model: BertModel, tokenizer:BertTokenizer, label_map: dict, 
    ...
    for batch in eval_dataloader:
        ...
        b_input_ids, b_input_mask, b_labels, b_predicates = batch
